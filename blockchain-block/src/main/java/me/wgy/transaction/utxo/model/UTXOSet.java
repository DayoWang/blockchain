...
public class UTXOSet {
  ...
  public SpendableOutputResult findSpendableOutputs(byte[] pubKeyHash, int amount) {
    ...
    int accumulated = 0;
    Map<String, byte[]> chainstateBucket = RocksDBStore.getInstance().getChainstateBucket();
  ...
  public void reIndex() {
    ...
    RocksDBStore.getInstance().cleanChainStateBucket();
    Map<String, TXOutput[]> allUTXOs = blockchain.findAllUTXOs();
    for (Map.Entry<String, TXOutput[]> entry : allUTXOs.entrySet()) {
      RocksDBStore.getInstance().putUTXOs(entry.getKey(), entry.getValue());
    }
   ...
   * 更新UTXO池
   * <p>
   * 当一个新的区块产生时，需要去做两件事情： 1）从UTXO池中移除花费掉了的交易输出； 2）保存新的未花费交易输出；
   *
    ...
    for (Transaction transaction : tipBlock.getTransactions()) {
      ...
      if (!transaction.isCoinbase()) {
        for (TXInput txInput : transaction.getInputs()) {
          ...
          TXOutput[] txOutputs = RocksDBStore.getInstance().getUTXOs(txId);
          ...
          if (remainderUTXOs.length == 0) {
            RocksDBStore.getInstance().deleteUTXOs(txId);
          } else {
            RocksDBStore.getInstance().putUTXOs(txId, remainderUTXOs);
          }
        }
      }
      ...
      RocksDBStore.getInstance().putUTXOs(txId, txOutputs);
    }
