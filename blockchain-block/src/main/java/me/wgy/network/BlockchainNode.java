...
import me.wgy.network.message.InvTypeEnum;
import me.wgy.network.message.MessageData;
import me.wgy.network.message.MessageTypEnum;
...
public class BlockchainNode extends Node {
  ...
  private class BaseHandler implements HandlerInterface {
    ...
    public MessageData getMsgData(PeerMessage peerMessage) {
      ...
      if (MessageTypEnum.JOIN.type.equals(msgType)) {
        return (MessageData.JoinMessageData) SerializeUtils
            .deserialize(peerMessage.getMsgDataBytes());
      }
      if (MessageTypEnum.VERSION.type.equals(msgType)) {
        return (MessageData.VersionMessageData) SerializeUtils
            .deserialize(peerMessage.getMsgDataBytes());
      }
      if (MessageTypEnum.GETBLOCKS.type.equals(msgType)) {
        return (MessageData.GetBlocksMessageData) SerializeUtils
            .deserialize(peerMessage.getMsgDataBytes());
      }
      if (MessageTypEnum.INVENTORY.type.equals(msgType)) {
        return (MessageData.InventoryMessageData) SerializeUtils
            .deserialize(peerMessage.getMsgDataBytes());
      }
      if (MessageTypEnum.GETDATA.type.equals(msgType)) {
        return (MessageData.GetDataMessageData) SerializeUtils
            .deserialize(peerMessage.getMsgDataBytes());
      }
      if (MessageTypEnum.BLOCK.type.equals(msgType)) {
        return (MessageData.BlockMessageData) SerializeUtils
            .deserialize(peerMessage.getMsgDataBytes());
      }
      if (MessageTypEnum.TX.type.equals(msgType)) {
        return (MessageData.TxMessageData) SerializeUtils
            .deserialize(peerMessage.getMsgDataBytes());
      }
  ...
  private class JoinHandler extends BlockchainNode.BaseHandler {
    ...
    public void handleMessage(PeerConnection peerConn, PeerMessage peerMessage,
      ...
      MessageData.JoinMessageData messageData = (MessageData.JoinMessageData) this
          .getMsgData(peerMessage);
  ...
  private class VersionHandler extends BlockchainNode.BaseHandler {
    ...
    public void handleMessage(PeerConnection peerConn, PeerMessage peerMessage,
      ...
      MessageData.VersionMessageData messageData = (MessageData.VersionMessageData) this
          .getMsgData(peerMessage);
  ...
  private class InventoryHandler extends BlockchainNode.BaseHandler {
    ...
    public void handleMessage(PeerConnection peerConn, PeerMessage peerMessage,
      ...
      MessageData.InventoryMessageData inventoryMessageData = (MessageData.InventoryMessageData) this
          .getMsgData(peerMessage);
  ...
  private class GetDataHandler extends BlockchainNode.BaseHandler {
    ...
    public void handleMessage(PeerConnection peerConn, PeerMessage peerMessage,
        Blockchain blockchain) {
      MessageData.GetDataMessageData getDataMessageData = (MessageData.GetDataMessageData) this
          .getMsgData(peerMessage);
  ...
  private class BlockHandler extends BlockchainNode.BaseHandler {
    ...
    public void handleMessage(PeerConnection peerConn, PeerMessage peerMessage,
        Blockchain blockchain) {
      MessageData.BlockMessageData blockMessageData = (MessageData.BlockMessageData) this
          .getMsgData(peerMessage);
  ...
  private class TxHandler extends BlockchainNode.BaseHandler {
    ...
    public void handleMessage(PeerConnection peerConn, PeerMessage peerMessage,
        Blockchain blockchain) {
      MessageData.TxMessageData txMessageData = (MessageData.TxMessageData) this
          .getMsgData(peerMessage);
    ...
    private void minerTx(PeerConnection peerConn, Blockchain blockchain, PeerInfo myPeerInfo,
        String miningAddress) {
      ...
      // 挖矿奖励
      verifyTx.add(Transaction.createCoinbaseTX(miningAddress, ""));
