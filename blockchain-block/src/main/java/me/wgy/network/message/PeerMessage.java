...
public class PeerMessage<T extends MessageData> {
  ...
  /**
   * 以字节数组的形式返回此消息的压缩表示形式： type + data length + data
   */
  public byte[] toBytes() {
    ...
    // 保存消息类型byte数组
    for (int i = 0; i < MESSAGE_TYPE_DATA_LENGTH; i++) {
      bytes[i] = type[i];
    }
    // 保存消息数据长度byte数组
    for (int i = 0; i < 4; i++) {
      bytes[i + MESSAGE_TYPE_DATA_LENGTH] = dataLenBytes[i];
    }
    // 保存消息数组byte数组
    for (int i = 0; i < data.length; i++) {
      bytes[i + 4 + MESSAGE_TYPE_DATA_LENGTH] = data[i];
    }
