...
public class TransactionBlock {
  ...
  public String calculateHash() {
    StringBuffer input = new StringBuffer().append(previousHash).append(merkleRoot)
        .append(timeStamp).append(nonce);
    return EncryptTool.applySha256(input.toString());
  }

  /**
   * 工作量证明（Proof-of-Work） 验证hash，直到块的hash以difficulty个0开头
   *
  ...
  public void ProofOfWork(int difficulty) {
    ...
    while (!hash.substring(0, difficulty).equals(target)) {
      // 更新nonce可以不断通过calculateHash方法生成所需的哈希值
      nonce++;
      hash = calculateHash();
    }
